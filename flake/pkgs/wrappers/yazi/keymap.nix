# https://yazi-rs.github.io/docs/configuration/keymap
# https://github.com/sxyazi/yazi/blob/shipped/yazi-config/preset/keymap-default.toml
{ lib, setWallpaperCommand }:
let
  inherit (lib.strings) optionalString;
in
# toml
''
  [mgr]
  prepend_keymap = [
    { on = "l", run = "plugin bypass smart-enter", desc = "Open a file, or recursively enter child directory, skipping children with only a single subdirectory" },
    { on = "h", run = "plugin bypass reverse", desc = "Recursively enter parent directory, skipping parents with only a single subdirectory" },
    { on = "p", run = "plugin smart-paste", desc = "Paste into the hovered directory or CWD" },
    { on = "T", run = "plugin toggle-pane min-preview", desc = "Hide or show preview" },
    { on = "C", run = "plugin ouch", desc = "Compress with ouch" },
    { on = [ "g", "e" ], run = "arrow bot", desc = "Go to bottom" },
    # { on = "<backspace>", run = "remove", desc = "Trash selected files" },
    { on = "<backspace>", run = "remove --permanently", desc = "Permanently delete selected files" },
    { on = "<C-z>", run = "noop" },
    { on = [ "g", "c" ], run = "noop" }, # run = "cd $XDG_CONFIG_HOME", desc = "Go to the config directory" },
    { on = [ "g", "d" ], run = "noop" }, # run = "cd $XDG_DOWNLOAD_DIR", desc = "Go to the downloads directory" },

    { on = "o", run = "shell 'plumber \"$1\"' --confirm", desc = "Open selected files" },
    { on = "<Enter>", run = "shell 'plumber \"$1\"' --confirm", desc = "Open selected files" },
    ${optionalString (setWallpaperCommand != null)
      ''{ on = "W", run = "shell '${setWallpaperCommand} \"$1\"' --confirm", desc = "Set wallpaper" },''
    }
    { on = "<C-n>", run = "shell 'dragon-drop -x -T \"$1\"' --confirm" },
  ]
''
